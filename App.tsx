  ...
  return (
    <div className="flex h-screen w-screen bg-gray-900 text-gray-100 font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col bg-gray-800 min-w-0">
        <header className="bg-gray-900/80 backdrop-blur-sm p-3 border-b border-gray-700 shadow-lg z-10">
            <div className="flex items-center justify-between">
                ...
                <h1 className="text-lg sm:text-xl font-bold tracking-wider text-cyan-400 text-center flex-grow truncate px-2">
                    {allianceName || '連盟'} {zoneName || '戦域'} 配置図
                </h1>
                <div className="w-7 lg:hidden" aria-hidden="true"></div>
            </div>
            <p className="text-xs text-gray-400 text-center mt-1 hidden sm:block">
                メンバーやゾーンをリストからマップへドラッグ＆ドロップして配置します。
            </p>
            {visibleCoords && (
                <p className="text-sm text-gray-500 font-mono mt-1 text-center">
                    表示範囲: {visibleCoords}
                </p>
            )}
        </header>
